...
//��������� ��������� � ���������� �������
$sTableID = "tbl_kit_mailing_event"; // ID �������
$oSort = new CAdminSorting($sTableID, "ID", "asc"); // ������ ����������
$lAdmin = new CAdminList($sTableID, $oSort); // �������� ������ ������

$cData = new CKitMailingEvent;
$sectData = new CKitMailingSectionTable;
...
//������� �����
//$sThisSectionUrl = "kit_mailing_list.php?parent=".$parentID."&lang=".LANG;;

//��� �������� �������� �� ������
$sLastFolder = '';

if($parentID>0) {
    $rsData = CKitMailingSectionTable::GetList(array("filter" => array("ID"=>$parentID)));
    while($arRes = $rsData->Fetch()){    
        $sLastFolder = "kit_mailing_list.php?parent=".$arRes["CATEGORY_ID"]."&lang=".LANG;
    }
}
...
//$cDataMessage = new CKitMailingMessage;
// ******************************************************************** //
//                ��������� �������� ��� ���������� ������              //
// ******************************************************************** //
...
if($lAdmin->EditAction() && $POST_RIGHT=="W") {
    ...
    foreach($FIELDS as $ID=>$arFields)
    {
        ...
        $ID = intval(substr($ID,1));
            ...
            if($type == "R"){
                if(!$cData->Update($ID, $arFields))
                {
                    ...
                    //������� ��������� ��������� ������� bitrix
                    $res = CKitMailingEvent::GetByID($ID);
        ...
        if($res = CKitMailingEvent::GetByID($ID)) {
                ...
                {           
                   case "delete":
                        ...
                        if($TYPE=="S") {
                            ...
                            CKitMailingSectionTable::Delete($ID);
                        }
                        
                        if($TYPE=="R"){
                            // ������ ��� ��������� ��������      
                            $resEventMessage = CKitMailingMessage::GetList(array(),array('ID_EVENT'=>$ID),false,array('ID'));
                            while($arrEventMessage = $resEventMessage->Fetch()) {
                                CKitMailingMessage::Delete($arrEventMessage['ID']);
                            }
                            
                                                                      
                            //������ ��� ������� ��������    
                            $resEventTemplate = CKitMailingMessageTemplate::GetList(array(),array('ID_EVENT'=>$ID),false,array('ID','ID_EVENT'));
                            while($arrEventTemplate = $resEventTemplate->Fetch()) {
                                CKitMailingMessageTemplate::Delete($arrEventTemplate['ID']);
                            }                     
...
while($arRes = $rsData->NavNext(true, "f_")):
        ...
        $editUrl = "kit_mailing_detail.php?T=".$f_T."&ID=".$f_ID."&KIT_MAILING_DETAIL=Y&parent=".$parentID."&lang=".LANG;
        $copyUrl = "kit_mailing_detail.php?T=".$f_T."&TEMPLATE=".$f_TEMPLATE."&ID_COPY=".$f_ID."&parent=".$parentID."&lang=".LANG;
       ...
       if($f_T=="S")
        {
            $row->AddViewField("NAME", '<a href="kit_mailing_list.php?parent='.$f_ID.'&lang='.LANG.'" class="adm-list-table-icon-link" style='.$style.' title='.$title.'><span class="adm-submenu-item-link-icon adm-list-table-icon iblock-section-icon"></span><span class="adm-list-table-link">'.$f_NAME.'</span></a>');
        }
        else{
            
            $row->AddViewField("NAME", '<a href="kit_mailing_detail.php?T='.$f_T.'&ID='.$f_ID.'&KIT_MAILING_DETAIL=Y&parent='.$parentID.'&lang='.LANG.'" title="'.GetMessage($module_id."_ACTION_EDIT").'">'.$f_NAME.'</a>');
         ...
         $arActions = array();
           ...
           if($f_T=="S"){
                $arActions[] = array(
                    ...
                    "ACTION"=>$lAdmin->ActionRedirect("kit_mailing_section_edit.php?parent=".$parentID."&ID=".$f_ID)
                );
                $arActions[] = array(
                    "ICON"=>"delete",
    ...
    if(is_array($arrTempl)) {
        ...
        foreach($arrTempl as $kres => $arRes) {
            ...
            $arDDMenu[] = array(
                ...
                //"ACTION" => "window.location = 'kit_mailing_detail.php?lang=".LANG."&TEMPLATE=".$arRes["NAME"]."';"
...
$aContext = array(
        ...
        array(
            ...
            "LINK"=>"kit_mailing_section_edit.php?parent=".$parentID."&lang=".LANG,
...
{
    $aContext[] = Array(
        "TEXT" => GetMessage($module_id."_KIT_MAILING_UP"),
        "LINK" => $sLastFolder,
        "TITLE" => GetMessage($module_id."_KIT_MAILING_UP_TITLE"),
    );
}
