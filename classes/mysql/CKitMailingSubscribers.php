<?
class CKitMailingSubscribers
{  
    public function Add($arFields,$SETTING)
    {
        global $DB;
        $arInsert = $DB->PrepareInsert("b_kit_mailing_subscribers", $arFields);
        $strSql =
            "INSERT INTO b_kit_mailing_subscribers(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
        ...
        if($arFields['CATEGORIES_ID']) {
            CKitMailingSubscribers::SubscribersAdd(array('SUBSCRIBERS_ID'=>$ID,'CATEGORIES_ID'=>$arFields['CATEGORIES_ID']),$SETTING);
        }
    ...
    public function Update($ID ,$arFields,$SETTING)
    {
        global $DB;
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_kit_mailing_subscribers", $arFields);  
        $strSql = "UPDATE b_kit_mailing_subscribers SET ".$strUpdate." WHERE ID=".$ID;
    ...
    public function checkSubscribers($arFields) {
        ...
        $dbres = CKitMailingSubscribers::GetList(array(),$fillter);
        $arRes  = array();
    ...
    public function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_kit_mailing_subscribers WHERE ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);
        
        //������ �������� ��� ������� ����������
         CKitMailingSubscribers::SubscribersDelete($ID);
    ...
    public function GetByID($ID)
    {
        global $DB;    
        $ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_kit_mailing_subscribers P WHERE P.ID='".$DB->ForSql($ID)."'";
    ...
    {
        global $DB;    
        $strSql = "SELECT P.* FROM b_kit_mailing_subscribers P WHERE P.EMAIL_TO='".$DB->ForSql($MAIL)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    } 
    ...
    public function SubscribersAdd($arFields, $SETTING)
    {
        ...
        //������� ��� ��������� � ������� ���� ���������
        //START
        $subsInfo = CKitMailingSubscribers::GetCategoriesBySubscribers($arFields['SUBSCRIBERS_ID']);
        //�������� �� ����������
            ...
            if($SETTING['ACTION_DELETE']!='NO_DELETE'){
                foreach($subsInfo as $k => $it) {
                    ...
                    if(!in_array($it, $arFields['CATEGORIES_ID'])){
                        CKitMailingSubscribers::SubscribersDeleteByID($k);
                    }  
                }                    
            }
        ...
        if(is_array($arFields['CATEGORIES_ID'])){
            foreach($arFields['CATEGORIES_ID'] as $categor) {
                ...
                $arInsert = $DB->PrepareInsert("b_kit_mailing_subscr_categ", array('SUBSCRIBERS_ID'=>$arFields['SUBSCRIBERS_ID'],'CATEGORIES_ID'=>$categor));        
                $strSql =
                    "INSERT INTO b_kit_mailing_subscr_categ(".$arInsert[0].") ".
                    "VALUES(".$arInsert[1].")";
                $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);                  
            }                
        }        
    ...
    public function SubscribersDelete($SUBSCRIBERS_ID)
    {
        ...
        $strSql = "DELETE FROM b_kit_mailing_subscr_categ WHERE SUBSCRIBERS_ID='".$DB->ForSql($SUBSCRIBERS_ID)."'";
        $DB->Query($strSql, true);   
        return true;       
    }    
    ...
    public function CategoriesDelete($CATEGORIES_ID)
    {
        ...
        $strSql = "DELETE FROM b_kit_mailing_subscr_categ WHERE CATEGORIES_ID='".$DB->ForSql($CATEGORIES_ID)."'";
        $DB->Query($strSql, true);   
        return true;       
    }     
    //������� �������� �� ID
    public function SubscribersDeleteByID($ID)
    {
        global $DB;   
        $ID = IntVal($ID); 
        $strSql = "DELETE FROM b_kit_mailing_subscr_categ WHERE ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);   
        return true;       
    }    
    ...
    public function SubscribersCategoriesDelete($SUBSCRIBERS_ID, $CATEGORIES_ID)
    {
        ...
        $SUBSCRIBERS_ID = IntVal($SUBSCRIBERS_ID); 
        $CATEGORIES_ID = IntVal($CATEGORIES_ID); 
        $strSql = "DELETE FROM b_kit_mailing_subscr_categ WHERE SUBSCRIBERS_ID='".$DB->ForSql($SUBSCRIBERS_ID)."' AND CATEGORIES_ID='".$DB->ForSql($CATEGORIES_ID)."'";
        $DB->Query($strSql, true);   
        return true;       
    }     
    ...
    {
        global $DB;    
        $strSql = "SELECT P.* FROM b_kit_mailing_subscr_categ P WHERE P.CATEGORIES_ID='".$DB->ForSql($CATEGORIES_ID)."'";
        $dbRes = $DB->Query($strSql, true);
    ...
    public function GetCategoriesBySubscribers($SUBSCRIBERS_ID)
    {
        global $DB;    
        $SUBSCRIBERS_ID = IntVal($SUBSCRIBERS_ID); 
        $strSql = "SELECT P.* FROM b_kit_mailing_subscr_categ P WHERE P.SUBSCRIBERS_ID='".$DB->ForSql($SUBSCRIBERS_ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = array();
        while($arResitem = $dbRes->Fetch()) {
    ...
    {
        global $DB;    
        $strSql = "SELECT P.* FROM b_kit_mailing_subscr_categ P";
        $dbRes = $DB->Query($strSql, true);
    ...
    public function GetList($aSort=Array(), $arFilter=Array(), $arNavStartParams=false, $arSelect=Array())
    {
        ...
        if($arFilter['CATEGORIES_ID'] && empty($arFilter['ID'])){
            ...
            foreach($arFilter['CATEGORIES_ID'] as $categ) {
                $categorSubscribers = CKitMailingSubscribers::GetSubscribersByCategory($categ);
...
//        $strSqlSearch['ID'] = 2;
        ...
        $strSql = "
            SELECT 
                 ".$strSqlSelect."            
            FROM b_kit_mailing_subscribers P
            WHERE
            ".$strSqlSearch."
        ";   
