...
// START
$categoriesList = CKitMailingHelp::GetCategoriesInfo();
// END

$sTableID = "tbl_kit_mailing_subscribers"; // ID �������
...
$cData = new CKitMailingSubscribers;
// ******************************************************************** //
//                ��������� �������� ��� ���������� ������              //
// ******************************************************************** //
                ...
                switch($_REQUEST['action'])
                {
                   ...
                   case "add_categories":
                        if($_REQUEST['categories_to_move']){
                            CKitMailingSubscribers::SubscribersAdd(array('SUBSCRIBERS_ID'=>$ID,'CATEGORIES_ID'=>$_REQUEST['categories_to_move']),array('ACTION_DELETE'=>'NO_DELETE'));
                        }     
                   break;                   
                    // ������� �� ������
                   case "delete_categories":
                        if($_REQUEST['categories_to_move']){
                            CKitMailingSubscribers::SubscribersCategoriesDelete($ID, $_REQUEST['categories_to_move']);
                        }     
                   break; 
...
while($arRes = $rsData->NavNext(true, "f_", false)) {
    $editUrl = "kit_mailing_subscribers_detail.php?ID=".$f_ID."&lang=".LANG;
    ...
    if($POST_RIGHT>="W") {           
         ...
         $row->AddViewField("EMAIL_TO", '<a href="kit_mailing_subscribers_detail.php?ID='.$f_ID.'&amp;lang='.LANG.'" title="'.GetMessage("subscr_upd").'">'.$f_EMAIL_TO.'</a>');
        ...
        if($f_USER_ID > 0 && $arUsers[$f_USER_ID]) {
         ...
         //������� ��������� ��������
         $catInfo = CKitMailingSubscribers::GetCategoriesBySubscribers($f_ID);
         $row_CATEGORIES_ID = '';
         if($catInfo) {
            foreach($catInfo as $c) {
                if($categoriesLi[$c]) {
                    $row_CATEGORIES_ID .= '<p style="margin: 0px 0px 4px 0px;">[<a target="_blank" href="kit_mailing_categories_detail.php?ID='.$categoriesLi[$c]['ID'].'&KIT_MAILING_DETAIL=Y&lang='.LANG.'">'.$categoriesLi[$c]['ID'].'</a>] '.$categoriesLi[$c]['NAME'].'</p>'  ;
                } 
            }    
         }  
...
if($POST_RIGHT>="W") {
    ...
    //START
    $arrGroup = array();
    $dbGroup = CKitMailingCategories::GetList(array('ID'=>'asc'),array(),false,array('ID','ACTIVE','NAME'));
...
if($POST_RIGHT>="W") {
    ...
    $aContext = array(
        array(
            ...
            "ICON" => "btn_new",
            "LINK" => "kit_mailing_subscribers_detail.php?lang=".LANG,
