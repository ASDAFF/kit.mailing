...
if($_GET['ID']) {
    ...
    $arResult = CKitMailingMessage::GetByID($ID);
    
    if(empty($arResult)){
        LocalRedirect("kit_mailing_message.php?lang=".LANG, true);
    } 
    ...
    //���������� �������� ���������� ��� ������
    $InfoSend = CKitMailingMessage::GetByIDInfoSend($arResult['ID']);
...
if($_GET['action'] == 'send') {
    CKitMailingTools::SendMessage(array('ID' =>$ID));   
    LocalRedirect("kit_mailing_message_detail.php?ID=".$ID."&lang=".LANG, true);
}
...
if($_REQUEST['update'] && $_REQUEST['save']){
     ...
     if($ID) { 
        CKitMailingMessage::Update($ID , $arr_save);    
     } 
...
$aMenu = array(
    array(
        ...
        "ICON" => "btn_list",
        "LINK" => "/bitrix/admin/kit_mailing_message.php?lang=".LANG
    )
);
...
if($ID) {
    ...
    if($arResult['SEND'] == 'N') {
        ...
        $aMenu[] = array(
            ...
            "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_SEND_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_message_detail.php?action=send&ID=".$ID."&lang=".LANG."&".bitrix_sessid_get()."#tb';",
            "ICON"=>"btn_start_send"
        );         
    }
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_DELETE_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_message.php?action=delete&ID[]=".$ID."&lang=".LANG."&".bitrix_sessid_get()."';",
