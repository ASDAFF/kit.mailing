...
//START
    ...
    {   
        ...
        if(strpos($arSect['SECTION_PAGE_URL'],'?')) {
            $arSect['SECTION_PAGE_URL'] = $arParams['MAILING_SITE_URL'].$arSect['SECTION_PAGE_URL'].'&MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=kit_mailing_'.$arParams['MAILING_EVENT_ID'];
        } else {
            $arSect['SECTION_PAGE_URL'] = $arParams['MAILING_SITE_URL'].$arSect['SECTION_PAGE_URL'].'?MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=kit_mailing_'.$arParams['MAILING_EVENT_ID'];
        }     
    ...
    foreach($Novelty_section_id as $section_id) {
        ...
        if(count($arrNoveltyProduct) >= $arParams['TOP_COUNT_FILLTER_NOVELTY_GOODS_FROM']) {
            ...
            $NOVELTY_GOODS_PRODUCT[$section_id] = CKitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_TOP'] , $Novelty_section_id_INFO[$section_id]);
            foreach($arrNoveltyProduct as $ItemNoveltyProduct) {
                ...
                $NOVELTY_GOODS_PRODUCT[$section_id] .= CKitMailingHelp::ReplaceVariables($arParams["TEMP_NOVELTY_GOODS_LIST"] , $ItemNoveltyProduct);
                $i++;           
            }
            $NOVELTY_GOODS_PRODUCT[$section_id] .= CKitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_BOTTOM'] , $Novelty_section_id_INFO[$section_id]);
        }    
    ...
    $arrEmailSend = array(); 
    $dbSubscribers = CKitMailingSubscribers::GetList(array(),array('CATEGORIES_ID'=>$FillterMailingRazdel,'ACTIVE'=>'Y'), false, array('ID','USER_ID','EMAIL_TO'));
