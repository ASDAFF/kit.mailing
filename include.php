<?php
$_1544658314 = 'kit.mailing';
IncludeModuleLangFile(__FILE__);
CModule::IncludeModule('iblock');
$_42492588 = CModule::AddAutoloadClasses('kit.mailing',
    array(
        'CKitMailingHelp' => 'classes/general/CKitMailingHelp.php',
        'CModuleOptions' => 'classes/general/CModuleOptions.php',
        'CMailingDetailOptions' => 'classes/general/CMailingDetailOptions.php',
        'CKitMailingEvent' => 'classes/mysql/CKitMailingEvent.php',
        'CKitMailingMessage' => 'classes/mysql/CKitMailingMessage.php',
        'CKitMailingMessageText' => 'classes/mysql/CKitMailingMessageText.php',
        'CKitMailingMessageTemplate' => 'classes/mysql/CKitMailingMessageTemplate.php',
        'CKitMailingUnsubscribed' => 'classes/mysql/CKitMailingUnsubscribed.php',
        'CKitMailingUndelivered' => 'classes/mysql/CKitMailingUndelivered.php',
        'CKitMailingSubscribers' => 'classes/mysql/CKitMailingSubscribers.php',
        'CKitMailingCategories' => 'classes/mysql/CKitMailingCategories.php',
        'CkitMailingSubTools' => 'classes/general/CkitMailingSubTools.php',
        'CKitMailingSectionTable' => 'classes/mysql/CKitMailingSection.php',
        'MCAPI' => 'classes/general/MCAPI.class.php',
    )
);

class CKitMailingTools
{
    const EVENT_ADD_MAILING = "KIT_MAILING_EVENT_SEND";
    ...
    public function AgentStartTemplate($_50629441 = 0)
    {
        ...
        $_1381752207 = CKitMailingEvent::GetByID($_50629441);
        $_1584904758 = date('H');
        self::$_794221308 = true;
        if ($_1381752207['EVENT_PARAMS_AGENT_AROUND'] == 'Y') {
            CKitMailingTools::StartMailing($_50629441, array('TYPE_StartMailing' => 'AGENT'));
        } elseif ($_1584904758 >= $_1381752207['AGENT_TIME_START'] && $_1584904758 < $_1381752207['AGENT_TIME_END']) {
            CKitMailingTools::StartMailing($_50629441, array('TYPE_StartMailing' => 'AGENT'));
        }
        self::send();
        return "CKitMailingTools::AgentStartTemplate($_50629441);";
    }
    ...
    public function StartMailing($_50629441 = 0, $_2043825558 = array())
    {
        ...
        if ($_1381752207['ACTIVE'] == 'Y') {
            ...
            if ($_1381752207['COUNT_RUN'] != 0) {
                CKitMailingHelp::ProgressFileDelete($_50629441, $_1381752207['COUNT_RUN'] - 1);
            }
            if ($_1381752207['MAILING_WORK'] == 'N') {
                ...
                CKitMailingHelp::ProgressFile($_50629441, $_1381752207['COUNT_RUN'], $_529922068);
            }
            ...
            $_500120612['MAILING_PACKAGE_COUNT'] = COption::GetOptionString('kit.mailing', 'MAILING_PACKAGE_COUNT', '3000');
            unset($_500120612['MAILING_INFO']['TEMPLATE_PARAMS']);
            if ($_2043825558) {
                $_500120612['MORE_OPTIONS_TEMPLATE'] = $_2043825558;
            }
            $APPLICATION->IncludeComponent('kit:kit.mailing.logic', $_1381752207['TEMPLATE'], $_500120612);
            ...
            CKitMailingEvent::Update($_50629441, array('COUNT_RUN' => $_1381752207['COUNT_RUN'] + 1, 'DATE_LAST_RUN' => Date($DB->DateFormatToPHP(CLang::GetDateFormat('FULL', SITE_ID))), 'MAILING_WORK' => 'N', 'MAILING_WORK_COUNT' => 0, 'MAILING_WORK_PARAM' => ''));
            $_1489970456 = CKitMailingMessageTemplate::GetList(array('COUNT_START' => 'DESC'), array('ID_EVENT' => $_50629441, 'ARCHIVE' => 'N'), false, array('ID', 'COUNT_END'));
            while ($_2048944103 = $_1489970456->Fetch()) {
                CKitMailingMessageTemplate::Update($_2048944103['ID'], array('COUNT_END' => $_1381752207['COUNT_RUN'] + 1));
                break;
            }
        }
    }
    ...
    public static function send()
    {
        foreach (self::$_759810266 as $_1881468694) {
            CKitMailingTools::SendMessage(array('ID_EVENT' => $_1881468694['ID_EVENT'], 'ID' => $_1881468694['ID']));
            CKitMailingHelp::slaap(COption::GetOptionString('kit.mailing', 'MAILING_MESSAGE_SLAAP', '0.001'));
        }
    }
    ...
    {
        global $DB;
        $_319086815 = CModule::IncludeModuleEx('kit.mailing');
        if ($_319086815 == '0') {
            CKitMailingMessage::Update($_1822731620['ID'], array('SEND_ERROR' => GetMessage('NOT_FOUND')));
            return false;
        } elseif ($_319086815 == '3') {
            CKitMailingMessage::Update($_1822731620['ID'], array('SEND_ERROR' => GetMessage('DEMO_END')));
            return false;
        }
        if (empty($_1822731620['ID'])) {
            return false;
        }
        $_726319206 = CKitMailingMessage::GetByIDInfoSend($_1822731620['ID']);
        $_726319206['PARAM_MESSEGE'] = unserialize($_726319206['PARAM_MESSEGE']);
        $_1548927215 = '';
        $_644628512 = new CKitMailingHelp();
        if ($_726319206) {
        ...
        $_260962268 = CKitMailingHelp::GetMailingInfo();
        ...
        $_754080943['MESSEGE'] = CKitMailingHelp::ReplaceVariables($_2007401319, $_817130455);
        ...
        if ($_1171335915['EVENT_SEND_SYSTEM'] == 'BITRIX') {
            $_502923035 = CKitMailingHelp::GetSiteId();
            if ($_754080943['BCC']) {
            ...
            $_754080943['MESSEGE'] .= '<img src="' . $_1171335915['SITE_URL'] . '/bitrix/admin/kit_mailing_img.php?MAILING_MESSAGE=' . $_1822731620['ID'] . '" width="1px" height="1px"  />';
            $_516700919 = CEvent::Send(self::EVENT_ADD_MAILING, $_502923035, $_754080943, 'Y');
            if ($_516700919) {
                CKitMailingMessage::Update($_1822731620['ID'], array('SEND' => 'Y', 'DATE_SEND' => Date($DB->DateFormatToPHP(CLang::GetDateFormat('FULL', SITE_ID))), 'SEND_ERROR' => '', 'SEND_SYSTEM' => $_1171335915['EVENT_SEND_SYSTEM'],));
                return $_516700919;
            }
        }
        if ($_1171335915['EVENT_SEND_SYSTEM'] == 'UNISENDER') {
            ...
            if (stripos($_1170728781, '<') !== false) {
                ...
                $_296990361 = COption::GetOptionString('kit.mailing', 'UNSENDER_SENDER_NAME');
            }
            if (empty($_296990361)) {
                $_296990361 = COption::GetOptionString('kit.mailing', 'UNSENDER_SENDER_NAME');
            }
            if (empty($_353889608)) {
                $_353889608 = COption::GetOptionString('kit.mailing', 'UNSENDER_SENDER_EMAIL');
            }
            $_1794397518 = $_754080943['MESSEGE'] . ' <img src="' . $_1171335915['SITE_URL'] . '/bitrix/admin/kit_mailing_img.php?MAILING_MESSAGE=' . $_1822731620['ID'] . '" width="1px" height="1px"  />';
            $_2147447352 = array('sender_name' => $_296990361, 'sender_email' => $_353889608, 'email' => $_754080943['EMAIL_TO'], 'subject' => $_754080943['SUBJECT'], 'body' => $_1794397518, 'list_id' => $_1171335915['EVENT_SEND_SYSTEM_CODE'], 'user_campaign_id' => $_1822731620['ID_EVENT'], 'track_read' => 1, 'wrap_type' => COption::GetOptionString('kit.mailing', 'UNSENDER_WRAP_TYPE'),);
            $_1477231671 = CKitMailingHelp::QueryUniSender('sendEmail', $_2147447352);
            if ($_1477231671['error']) {
                CKitMailingMessage::Update($_1822731620['ID'], array('SEND_ERROR' => $_1477231671['error'], 'SEND_SYSTEM' => $_1171335915['EVENT_SEND_SYSTEM'],));
            }
            if ($_1477231671['result']['email_id']) {
                CKitMailingMessage::Update($_1822731620['ID'], array('SEND' => 'Y', 'SEND_SYSTEM_MESSEGE_CODE' => $_1477231671['result']['email_id'], 'DATE_SEND' => Date($DB->DateFormatToPHP(CLang::GetDateFormat('FULL', SITE_ID))), 'SEND_ERROR' => '', 'SEND_SYSTEM' => $_1171335915['EVENT_SEND_SYSTEM']));
                if ($_754080943['BCC']) {
                    $_502923035 = CKitMailingHelp::GetSiteId();
                    $_754080943['BCC'] = htmlspecialcharsBack($_754080943['BCC']);
                    $_1311136515 = $_754080943;
    ...
    public function AddMailingMessage($_1822731620 = array(), $_754080943 = array(), $_761901698 = array())
    {
        ...
        if (empty($_754080943['EMAIL_FROM']) || empty($_754080943['EMAIL_TO']) || empty($_754080943['SUBJECT'])) {
            $_529922068 = CKitMailingHelp::ProgressFileGetArray($_1822731620['MAILING_ID'], $_1171335915['COUNT_RUN']);
            $_529922068['COUNT_NOW'] = $_529922068['COUNT_NOW'] + 1;
            CKitMailingHelp::ProgressFile($_1822731620['MAILING_ID'], $_1171335915['COUNT_RUN'], $_529922068);
            return false;
        }
        $_817130455 = array();
        ...
        $_754080943['MESSEGE'] = CKitMailingHelp::ReplaceVariables($_754080943['MESSEGE'], $_817130455);
        ...
        CKitMailingHelp::ProgressFile($_1822731620['MAILING_ID'], $_1171335915['COUNT_RUN'], $_529922068, array('EMAIL_TO_SEND_INFO' => $_754080943['EMAIL_TO']));
        return $_74737845;
    }
    ...
    public function AgentMailingNeedAction()
    {
        ...
        CKitMailingTools::MailingNeedAction();
        return 'CKitMailingTools::AgentMailingNeedAction();';
    }
    ...
    public function MailingNeedAction()
    {
        ...
        $_1302794545 = CKitMailingSubscribers::GetList(array(), array('USER_ID' => '0'), false, array('ID', 'EMAIL_TO', 'USER_ID'));
        while ($_1321731202 = $_1302794545->Fetch()) {
            if ($_32705249[$_1321731202['EMAIL_TO']]) {
                CKitMailingSubscribers::Update($_1321731202['ID'], array('USER_ID' => $_32705249[$_1321731202['EMAIL_TO']]['ID']));
            }
        }
        $_1796246067 = CKitMailingEvent::GetList(array(), array('ACTIVE' => 'Y'), false, array('ID', 'TEMPLATE_PARAMS'));
        while ($_95748629 = $_1796246067->Fetch()) {
            ...
            if ($_761901698['NEW_COUPON_ADD'] == 'Y' && \Bitrix\Main\Loader::includeModule('sale')) {
                ...
                $_1376434809 = CKitMailingMessage::GetList(array(), $_1917078736, false, array('ID', 'PARAM_2', 'PARAM_MESSEGE'));
            ...
            if ($_761901698['COUPON_ADD'] == 'Y' && CModule::IncludeModule('catalog')) {
                ...
                $_558545227 = CKitMailingMessage::GetList(array(), $_1594570806, false, array('ID', 'PARAM_2', 'PARAM_MESSEGE'));
