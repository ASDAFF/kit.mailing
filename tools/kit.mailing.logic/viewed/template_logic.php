...
if($arParams["VIEWED_SHOW"] == 'Y' && CModule::IncludeModule("iblock") && CModule::IncludeModule("catalog")) {  
    ...
    if($arr_product_id) {
        ...
        if($arParams['INCLUDE_PHP_MODIF_VIEWED']=='Y'){ 
            include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/kit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_VIEWED_FILLTER_BEFORE.php");
        }        
        ...
        {      
            ...
            if($arParams['INCLUDE_PHP_MODIF_VIEWED']=='Y'){  
                include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/kit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_VIEWED_FILLTER_WHILE_AFTER.php");
            }
        ...
        foreach($arr_user_product_id as $kuser => $vproduct) {
            ...
            foreach($vproduct as $viewed_product_id) {
                if($arrViewedProduct[$viewed_product_id]){
                    ...
                    $VIEWED_ITEM_CONTENT .= CKitMailingHelp::ReplaceVariables($arParams["TEMP_LIST_VIEWED"] , $arrViewedProduct[$viewed_product_id]);
                    $i++;                          
                }        
            } 
            if($i>0) {
