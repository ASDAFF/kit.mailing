...
// $s = CComponentUtil::GetTemplatesList('sotbit:sotbit.mailing.logic',true);

// если шаблоны
/*
if(empty($_REQUEST['TEMPLATE']) && empty($_REQUEST['ID'])) {
    LocalRedirect("sotbit_mailing_list.php?lang=".LANG.GetFilterParams("filter_", false));
}
*/
...
//заменим ссылки при сохранении шаблона
//START
...
// обработаем параметры компонента
// START
...
if($_REQUEST['update'] && $_REQUEST['save'])
{
    ...
    //если время агентов поставили неправильно
    ...
    if($arResult['ID'])
    {
        ...
        // проверим шаблон и его состояние
        // START
        ...
        //Обновление шаблона в системе Bitrix
        //Получаем параметры почтового шаблона для обновления
        
        //Получаем идентификатор почтового шаблона, связанного с данной рассылкой
        $eventMessageId =  $helper->EventTemplateCheck($arrEventInfo['EVENT_TYPE'], $arrEventInfo['NAME']);
        //Подготовим данные для обновления почтового шаблона
    ...
    else
    {
        ...
        //При генерации нового почтового шаблона, данная проверка больше не требуется
        //$arr_save['EVENT_TYPE'] = CSotbitMailingHelp::EventMessageCheck();
        $arr_save['EVENT_TYPE'] = '';

        $arResult['ID'] = CSotbitMailingEvent::Add($arr_save);
        
        //Метод создает новый тип почтового события, возваращает тип созданного события
        $eventType =  $helper->CreateEventType($arResult['ID'], $arr_save['NAME']);
        
        //Создаем шаблон сообщения и привязываем к типу почтового события
        ...
        //Привяжем тип почтового события к рассылке
...
if($arResult['ID'] > 0)
{
    if($res = CSotbitMailingEvent::GetByID($arResult['ID']))
    {
        ...
        //работа с почтовым шаблоном
        //START

        //Проверка почтового события больше не требуется
        ...
        //Метод проверяет существование почтового события и шаблонов, связанных с ним, если их нет, то они создаются.
        $eventMessageId = $helper->EventTemplateCheck($res['EVENT_TYPE'], $res['NAME']);
        //Получаем данные почтового шаблона
        $rsEventMessage = CEventMessage::GetByID($eventMessageId);
        $arEventMessage = $rsEventMessage->Fetch();
        //END


        //работа с сервисом UniSender
        //START
        if($res['EVENT_SEND_SYSTEM'] == 'UNISENDER')
        {
            ...
            // если нет связанной рассылки, создадим новую
            ...
            //если отличается название, изменим
...
elseif($_GET['ID_COPY']) // если настроен функционал копирования
{
...
//список сценариев для дублирования
//START
...
//получим параметры шаблона компонента
...
// выставим значения которые будем показывать
...
// соберем группы рассылки которые есть
// START
if(is_array($paramTemplate))
{
...
// сформируем настройки шаблона
// START

/*
В массив  предварительно добавим данные почтового шаблона,
хранящиеся в массиве $arEventMessage
*/
...
if(is_array($paramTemplate))
{
    foreach($paramTemplate as  $KeyOpt => $templOption)
    {
        ...
        // сформируем массив значений для списка
        ...
        //если выбор из списка проверим множественность
...
<div id="status_bar" style="overflow:hidden;display:none; height: 40px;">
    ...
    <div class="adm-workarea" style="padding: 0px;">
        <a class="adm-btn" id="btn_start_stop" href="">Остановить рассылку</a>
    </div>
    */?>
</div>
...
function SotbitGetJsonProgress()
{
    ...
    $.getJSON('/bitrix/tmp/sotbit_mailing_progress_<?=$ID?>_<?=$arResult['COUNT_RUN']?>.json', function(data) {
        ...
        //запустим рассылку если остановилась
