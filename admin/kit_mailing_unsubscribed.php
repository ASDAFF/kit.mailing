...
$sTableID = "tbl_kit_mailing_unsubscribed"; // ID �������
...
$cData = new CKitMailingUnsubscribed;
// ******************************************************************** //
//                ��������� �������� ��� ���������� ������              //
// ******************************************************************** //
...
while($arRes = $rsData->NavNext(true, "f_", false)) {
    $editUrl = "kit_mailing_unsubscribed_detail.php?ID=".$f_ID."&lang=".LANG;  
    ...
    if($POST_RIGHT>="W") {           
         ...
         $row->AddViewField("EMAIL_TO", '<a href="kit_mailing_unsubscribed_detail.php?ID='.$f_ID.'&amp;lang='.LANG.'" title="'.GetMessage("subscr_upd").'">'.$f_EMAIL_TO.'</a>');
                  
         $row->AddCheckField("ACTIVE");

         $row_ID_MESSEGE = '<a target="_blank" href="kit_mailing_message_detail.php?ID='.$f_ID_MESSEGE.'&lang='.LANG.'">'.$f_ID_MESSEGE.'</a> ' ;                    
         $row->AddViewField("ID_MESSEGE", $row_ID_MESSEGE);
         
         
         $row_ID_EVENT = '[<a target="_blank" href="kit_mailing_detail.php?ID='.$f_ID_EVENT.'&KIT_MAILING_DETAIL=Y&lang='.LANG.'">'.$f_ID_EVENT.'</a>] '.$MailingList[$f_ID_EVENT]['NAME']  ;
...
if($POST_RIGHT>="W") {
    ...
    if(is_array($MailingList)) {   
        ...
        foreach($MailingList as $kres => $arRes) {
            ...
            $arDDMenu[] = array(
                ...
                "ACTION" => "window.location = 'kit_mailing_unsubscribed_detail.php?lang=".LANG."&MAILING=".$arRes["ID"]."';"
            ); 
    ...
    $aContext = array(
        ...
        array(
            ...
            "ICON" => "btn_new",
            "LINK" => "kit_mailing_unsubscribed_import.php?lang=".LANG,
