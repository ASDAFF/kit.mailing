...
if($_GET['MAILING']) {
    ...
    if(empty($MailingList[$_GET['MAILING']])) {
        LocalRedirect("sotbit_mailing_unsubscribed.php?lang=".LANG, true);
    }  
...
elseif($_GET['ID']) {
    ...
    $arResult = CSotbitMailingUnsubscribed::GetByID($ID);
...
if($_REQUEST['update'] && $_REQUEST['save']){
     ...
     LocalRedirect("sotbit_mailing_unsubscribed_detail.php?ID=".$ID."&lang=".LANG, true);
