...
if($_GET['ID']) {
    $ID = $_GET['ID'];
    $arResult = CKitMailingCategories::GetByID($ID);
    ...
    if(empty($arResult)){
        LocalRedirect("kit_mailing_categories.php?lang=".LANG, true);
    } 
...
if($_GET['ID'] && $arResult['SUNC_UNISENDER'] == 'Y' && $arResult['SUNC_UNISENDER_LIST'] && $_GET['action']=='EXPORT_UNISENDER') {
    $result = CKitMailingHelp::UniSenderExportContact($_GET['ID'],$arResult['SUNC_UNISENDER_LIST']);
    
    if($result['STATUS']=='OK') {
        LocalRedirect("kit_mailing_categories_detail.php?ID=".$ID."&UNISENDER_EXPORT_OK=".$result['COUNT']."&lang=".LANG);        
    }
...
if($_REQUEST['update'] && $_REQUEST['save']){
     ...
     if($ID) { 
            CKitMailingCategories::Update($ID , $arr_save);
     } else {
         global $DB ;
         ...
         if(empty($arr_save['NAME'])) {
            ...
            $ID = CKitMailingCategories::Add($arr_save);
         }
    ...
    // END     
    if($ID) {
        LocalRedirect("kit_mailing_categories_detail.php?ID=".$ID."&lang=".LANG, true);
    } 
...
$aMenu = array(
    array(
        ...
        "ICON" => "btn_list",
        "LINK" => "/bitrix/admin/kit_mailing_categories.php?lang=".LANG
    )
);
...
if($ID) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_EXPORT_UNISENDER_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_categories_detail.php?ID=".$ID."&action=EXPORT_UNISENDER&lang=".LANG."&".bitrix_sessid_get()."';",
    ...
    */
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_DELETE_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_categories.php?action=delete&ID[]=".$ID."&lang=".LANG."&".bitrix_sessid_get()."';",
...
$arOptions['TABS_VIDEO_INSTRUCT'] = array(
    ...
    'TYPE' => 'TABS_INFO', 
    'SORT' => '70',
    'DEFAULT' => GetMessage($module_id.'_TABS_KIT_MAILING_CATEGORY_INSTRUCTION'),
);  
