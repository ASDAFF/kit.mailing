...
<div class="kit_mailing_subscr_wrap">
    <div class="kit_mailing_subscr_wrap_in">
        <label><?=$arParams["INFO_TEXT"]?></label>
        <form id="kit_mailing_subsrib" method="post" action="#">
            <input type="text" value="<?=$arResult['SUBSCRIBER_INFO']['EMAIL_TO']?>" name="EMAIL_TO" title="<?=GetMessage('PANNEL_EMAIL')?>" placeholder="<?=GetMessage('PANNEL_EMAIL')?>" class="kit_mailing_subscr_wrap_email">
            <?if($arParams["CATEGORIES_SHOW"]=='Y' && count($arParams["CATEGORIES_ID"])>0):?>
                <div class="kit_mailing_subsrib_categories">
        ...
        BX.ajax({
            ...
            onsuccess: function(data){
                ...
                if(data.error == 'subscribed'){
                    document.querySelector('.kit_mailing_subscr_wrap_in form').remove();
                    document.querySelector('.kit_mailing_subscr_wrap_answer').innerHTML ="<?=GetMessage('SUBSCRIBE')?>";
                }else {
                    $('.kit_mailing_subscr_wrap_in form').remove();
                    $('.kit_mailing_subscr_wrap_answer').html('<?=$EMAIL_SEND_END?>');
                }
            },
        });         
