...
class CkitMailingSubTools {
    ...
    public function AddSubscribers($arFields,$SETTING) {
        ...
        $answer = CKitMailingSubscribers::checkSubscribers($arFields);
        if(is_array($answer)){ 
            if($arFields['CATEGORIES_ID']){ 
                ...
                CKitMailingSubscribers::SubscribersAdd(array('SUBSCRIBERS_ID'=>$answer['ID'],'CATEGORIES_ID'=>$arFields['CATEGORIES_ID']), array('ACTION_DELETE'=>$SETTING['ACTION_DELETE']));
            }
            return 'ERROR_SAVE';        
        }
        ...
        $ID_SUBSCRIBERS = CKitMailingSubscribers::add($arFields,[]);
        if($ID_SUBSCRIBERS) {
            ...
            if($PARAMETR_EVENT['USER']){
                if($PARAMETR_EVENT['USER']['ADD']=='Y'){
                    ...
                    if($USER_ID) {
                        ...
                        CKitMailingSubscribers::Update($ID_SUBSCRIBERS, array('USER_ID'=>$USER_ID));
            ...
            if($PARAMETR_EVENT['UNISENDER']['ADD'] && $PARAMETR_EVENT['UNISENDER']['LIST'] && $SETTING['UNISENDER_EXPORT']!='Y'){
                ...
                $getListUniSender = CKitMailingHelp::QueryUniSender('subscribe',$arrUnisender);
