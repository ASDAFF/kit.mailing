...
if($_REQUEST['update'] && $_REQUEST['save']){
     ...
     if($_REQUEST['TYPE']=='FILE_LOG' && $_REQUEST['FILE_LOG']){
        $parseLogInfo = CKitMailingHelp::ParseLogMail($_REQUEST['FILE_LOG']);
     ...
     $DBMailingUndelivered = CKitMailingUndelivered::GetList(array(),array(),false,array('ID','EMAIL_TO'));
     ...
     foreach($arr_email as $key => $email){
        ...
        if(empty($arrMailingUndelivered[$email])){
            CKitMailingUndelivered::Add($arr_save);
            $arr_answer['NEW']++;               
        } else {
            $arr_answer['OLD']++;    
            CKitMailingUndelivered::Update($arrMailingUndelivered[$email] , $arr_save);
        } 
     }
...
$aMenu = array(
    array(
        ...
        "ICON" => "btn_list",
        "LINK" => "/bitrix/admin/kit_mailing_undelivered.php?lang=".LANG
    )
);
...
if($ID) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_DELETE_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_undelivered.php?action=delete&ID[]=".$ID."&lang=".LANG."&".bitrix_sessid_get()."';",
...
if($arResult['TYPE']=='FILE_LOG' || $_REQUEST['TYPE']=='FILE_LOG'){  
    ...
    $arOptions['FILE_LOG'] = array(
        ...
        'TYPE' => 'STRING',
        'SORT' => '15',
        'DEFAULT' => COption::GetOptionString('kit.mailing','FILE_LOG','/var/log/maillog')
    );    
...
/*
$se = CKitMailingHelp::ParseLogMail();
printr($se);  
*/
?> 
