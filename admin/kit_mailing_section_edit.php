...
{       
    ...
    if($ID>0)
    {
        ...
        $result = CKitMailingSectionTable::Update($ID, $arFields);
        if (!$result->isSuccess())
        {
    ...
    else
    {
        ...
        $result = CKitMailingSectionTable::add($arFields);
    ...
    {
        if($apply!="")
            LocalRedirect("/bitrix/admin/kit_mailing_section_edit.php?ID=".$ID."&parent=".$parentID."&lang=".LANG, true);
        else
            LocalRedirect("/bitrix/admin/kit_mailing_list.php?parent=".$parentID."&lang=".LANG, true);
    }
    else
    {
...
if(isset($_REQUEST["ID"]) || $copy)
{
    ...
    $ID = (int)$_REQUEST["ID"];
    if($copy){
            $arDataTable = CKitMailingSectionTable::GetByID($copy)->Fetch();
    }   
    else {
        $arDataTable = CKitMailingSectionTable::GetByID($ID)->Fetch();
    }
...
$aMenu = array(
    array(
        ...
        "LINK"=>"kit_mailing_list.php?parent=".$parentID."&lang=".LANG,
        "ICON"=>"btn_list",
    )
);
if($ID>0)
{
    ...
    $aMenu[] = array(
        ...
        "LINK"=>"kit_mailing_section_edit.php?parent=".$parentID."&copy=".$ID."&lang=".LANG,
        "ICON"=>"btn_copy",
    );
    $aMenu[] = array(
        "TEXT"=>GetMessage($module_id."_section_delete"),
        "TITLE"=>GetMessage($module_id."_section_delete_title"),
        "LINK"=>"javascript:if(confirm('".GetMessage($module_id."_section_delete_title")."'))window.location='kit_mailing_list.php?ID=S".$ID."&action=delete&parent=".$parentID."&lang=".LANG."&".bitrix_sessid_get()."';",
        "ICON"=>"btn_delete",
    );
}
...
$tabControl->Buttons(
    array(
        ...
        "back_url"=>"kit_mailing_list.php?parent=".$parentID."&lang=".LANG,
