...
// Получим данные о рассылках
// START
$MailingList = CSotbitMailingHelp::GetMailingInfo();
// END

$sTableID = "tbl_sotbit_mailing_static"; // ID таблицы
$oSort = new CAdminSorting($sTableID, "DATE_SEND", "asc"); // объект сортировки
...
// ограничим статистику на 3 месяца
...
//получим почты всех пользователей
//START
$arrAllUser = array();
...
while($arRes = $rsData->Fetch())
{
    ...
    //Общий переход пользователей к нам на сайт
    //START
    ...
    //переход пользователей к нам на сайт из открытых писем
    //START
    ...
    //время дня отправки писем и даты когда
    //START
    if($arRes['DATE_SEND'])
    {
    ...
    {
        //строки которые надо обрезать
        $strReplaceLink = array(
            'utm_source=email&',
            'utm_source=email',
            'utm_medium=sotbit_mailing&',
            'utm_medium=sotbit_mailing',
            ...
            'utm_medium=email&',
            'utm_medium=email',
            'utm_campaign=sotbit_mailing_' . $arRes['ID_EVENT'] . '&',
            'utm_campaign=sotbit_mailing_' . $arRes['ID_EVENT'],
            'MAILING_MESSAGE=' . $arRes['ID'] . '&',
            'MAILING_MESSAGE=' . $arRes['ID']
        );
        //если есть авторизация добавим замену
        ...
        foreach($arRes['STATIC_PAGE_START'] as $klink => $vlink)
        {
            ...
            //если последний знак ?  уберем его
            if(substr($vlink, -1) == '?')
            {
                $vlink = str_replace('?', "", $vlink);
            }
            //если последняя лямба
            if(substr($vlink, -1) == '&')
            {
    ...
    // соберем домены получателей писем
    // START
    if($arRes['EMAIL_TO'])
    {
        ...
        //открываемость писем по доменам
        if($arRes['STATIC_USER_OPEN'] == 'Y')
        {
    ...
    if($arRes['PARAM_MESSEGE']['COUPON'])
    {
        ...
        if($arRes['DATE_SEND'])
        {
            //получим количество выдачи купонов по дате
            $dateParse = ParseDateTime($arRes['DATE_SEND']);
    ...
    // получим всех пользователей из сценариев
    // START
    ...
    //получим все заказы по сообщениям
    //START
...
//получим купоны  из системы и соберем статистику
//START
...
//получим заказы и статистику по ним
//START
$arrOrder = array();
$arrOrderId = array();
if(CModule::IncludeModule("sale") && $arrStaticInfo['MAILING_USER_ID'])
{
    ...
    while($arItemsOrder = $dbResOrder->Fetch())
    {
        ...
        //выборка закаов для пользователей
        $arrOrder[$arItemsOrder['ID']] = $arItemsOrder;
    }
    //END

    //получим товары для заказов
    //START
    $basketFillter = array(
        // 'ORDER_ID' => $arrOrderId
    );
...
//сформируем данные для статистики заказов
//START
foreach($arrOrder as $arItemsOrder)
{
    ...
    //получим количество заказов по дате
    ...
    //получим стоимость всех заказов
...
//пройдемся по датам и если там по одной дате обновим
// START
