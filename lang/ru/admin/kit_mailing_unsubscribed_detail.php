<?
$module_id = 'kit.mailing';
$MESS[$module_id."_PAGE_TITLE"] = "E-mail #ID#";

$MESS[$module_id."_DEMO"] = "Модуль работает в демо-режиме.";
$MESS[$module_id."_DEMO_DETAILS"] = 'Приобрести полнофункциональную версию вы можете по адресу: <a href="http://marketplace.1c-bitrix.ru/solutions/kit.mailing/" target="_blank">http://marketplace.1c-bitrix.ru/solutions/kit.mailing</a>';
$MESS[$module_id."_DEMO_END"] = "Демо-режим закончен.";
$MESS[$module_id."_DEMO_END_DETAILS"] = 'Приобрести полнофункциональную версию вы можете по адресу: <a href="http://marketplace.1c-bitrix.ru/solutions/kit.mailing/" target="_blank">http://marketplace.1c-bitrix.ru/solutions/kit.mailing</a>';

$MESS[$module_id."_edit10"] = "Отписавшийся e-mail";  

$MESS[$module_id."_OPTION_DEF_NANE"] = "Настройки";
 

$MESS[$module_id."_OPTION_10"] = "Отписавшийся e-mail"; 




$MESS[$module_id."_ID_MESSEGE_TITLE"] = "ID сообщения"; 
$MESS[$module_id."_ID_EVENT_TITLE"] = "Рассылка"; 
$MESS[$module_id."_DATE_CREATE_TITLE"] = "Дата создания"; 
$MESS[$module_id."_ACTIVE_TITLE"] = "Активность"; 

$MESS[$module_id."_EMAIL_TO_TITLE"] = "E-mail";               
$MESS[$module_id."_SUBJECT_TITLE"] = "Заголовок письма";      
$MESS[$module_id."_MESSEGE_TITLE"] = "Сообщение";   
    
    
$MESS[$module_id."_DATE_LAST_RUN_TITLE"] = "Последний запуск рассылки"; 
$MESS[$module_id."_TEMPLATE_TITLE"] = "Тип рассылки"; 


    
$MESS[$module_id."_VALUE_Y"] = "да";         
$MESS[$module_id."_VALUE_N"] = "нет";  
 
$MESS[$module_id."_PANEL_TOP_BACK_TITLE"] = "Список отписавшихся e-mail";  

$MESS[$module_id."_PANEL_TOP_DELETE_TITLE"] = "Удалить"; 
$MESS[$module_id."_PANEL_TOP_DELETE_ALT"] = "Удалить отписавшийся email из базы"; 
$MESS[$module_id."_PANEL_TOP_DELETE_CONFORM"] = "Вы уверены, что хотите удалить отписавшийся email №#ID# из базы?";


$MESS[$module_id."_PANEL_TOP_SEND_TITLE"] = "Отправить сообщение";
$MESS[$module_id."_PANEL_TOP_SEND_ALT"] = "Отправить сообщение получателю";
$MESS[$module_id."_PANEL_TOP_SEND_CONFORM"] = "Вы уверены, что хотите отправить сообщение №#ID# получателю?";

?>