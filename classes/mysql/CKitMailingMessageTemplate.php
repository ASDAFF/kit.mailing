<?
/**
* ����� ��� ������ � �������� b_kit_mailing_message_template, ������� ������ ������� �������� ��������� ��� ��������.
*
* ���� �������:
* <ul>
* <li><b>ID</b> - ������������� �������/li>
...
* </ul> 
*/
class CKitMailingMessageTemplate
{  
    public function Add($arFields)
    {
        global $DB;
        $arInsert = $DB->PrepareInsert("b_kit_mailing_message_template", $arFields);
        $strSql =
            "INSERT INTO b_kit_mailing_message_template(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
    ...
    public function Update($ID ,$arFields)
    {
        global $DB;
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_kit_mailing_message_template", $arFields);
        $strSql = "UPDATE b_kit_mailing_message_template SET ".$strUpdate." WHERE ID=".$ID;
    ...
    public function GetByID($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_kit_mailing_message_template P WHERE P.ID='".$DB->ForSql($ID)."'";
    ...
    public function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_kit_mailing_message_template WHERE ID='".$DB->ForSql($ID)."'";
    ...
    public function GetList($aSort=Array(), $arFilter=Array(), $arNavStartParams=false, $arSelect=Array())
    {
        ...
        if (is_array($arFilter))
        {
            foreach($arFilter as $key=>$val)
            {
                ...
                case "TEMPLATE":
                    ...
                    break; 
                case "AR�HIVE":
                    $arSqlSearch[] = GetFilterQuery("P.AR�HIVE",$val);                    
                    break;                                                                                                                                                              
                }                  
            }
        }
        ...
        {    
            ...
            switch($key)
            {
                ...
                case "AR�HIVE":    $arOrder[$key] = "P.AR�HIVE ".$ord; break;                       
            }        
        }
        if(count($arOrder) <= 0)
        {
        ...
        $strSql = "
            SELECT 
                 ".$strSqlSelect."            
            FROM b_kit_mailing_message_template P
            WHERE
            ".$strSqlSearch."
        ";   
