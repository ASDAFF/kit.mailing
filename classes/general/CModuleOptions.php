...
class CModuleOptions
{
    ...
    public function CModuleOptions($module_id, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
        ...
        if($need_access_tab) {
            $this->arTabs[] = array(
                'DIV' => 'edit_access_tab',
                'TAB' => GetMessage($this->module_id."_access_Tab"),
                'ICON' => '',
                'TITLE' => GetMessage($this->module_id."_access_title")
            );            
        }
    ...
    public function ShowHTML()
    {
        ...
        if(is_array($this->arOptions))
        {
                ...
                switch($arOptParams['TYPE'])
                {
                    ...
                    case 'TEXT':
                        ...
                        if(!isset($arOptParams['ROWS']))
                            $arOptParams['ROWS'] = 5;
                        $input = '<textarea rows="'.$type[1].'" cols="'.$arOptParams['COLS'].'" rows="'.$arOptParams['ROWS'].'" name="'.$opt.'">'.htmlspecialcharsEx($val).'</textarea>';
            ...
            echo     '<input type="hidden" name="update" value="Y" />
                    <input type="submit" name="save" value="' .GetMessage($this->module_id."_submit_save"). '" />
