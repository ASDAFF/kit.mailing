...
if($_GET['ID']) {
    $ID = $_GET['ID'];
    $arResult = CSotbitMailingSubscribers::GetByID($ID);
    $arResult['CATEGORIES_ID'] = CSotbitMailingSubscribers::GetCategoriesBySubscribers($_GET['ID']);  
    ...
    if(empty($arResult)){
        LocalRedirect("sotbit_mailing_subscribers.php?lang=".LANG, true);
    } 
...
if($_REQUEST['update'] && $_REQUEST['save']){
     ...
     if($ID) { 
        ...
        CSotbitMailingSubscribers::Update($ID, $arr_save, $SETTING);
        $STATUS = 'UPDATE';  
     } else {
         global $DB;

         $arr_save['SOURCE'] = 'ADMIN_ONE';                               
         $ANSWER = CsotbitMailingSubTools::AddSubscribers($arr_save,[]);   
     ...
     if($ID) {
        LocalRedirect("sotbit_mailing_subscribers_detail.php?ID=".$ID."&STATUS=".$STATUS."&lang=".LANG, true);
     }             
...
$aMenu = array(
    array(
        ...
        "ICON" => "btn_list",
        "LINK" => "/bitrix/admin/sotbit_mailing_subscribers.php?lang=".LANG
    )
);
...
if($ID) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_DELETE_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/sotbit_mailing_subscribers.php?action=delete&ID[]=".$ID."&lang=".LANG."&".bitrix_sessid_get()."';",
