<?
class CKitMailingMessageText
{  
    public function Add($arFields)
    {
        global $DB;
        $arInsert = $DB->PrepareInsert("b_kit_mailing_message_text", $arFields);
        $strSql =
            "INSERT INTO b_kit_mailing_message_text(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
    ...
    public function Update($ID ,$arFields)
    {
        global $DB;
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_kit_mailing_message_text", $arFields);
        $strSql = "UPDATE b_kit_mailing_message_text SET ".$strUpdate." WHERE ID=".$ID;
    ...
    public function GetByID($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_kit_mailing_message_text P WHERE P.ID='".$DB->ForSql($ID)."'";
    ...
    public function GetByMessegeID($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_kit_mailing_message_text P WHERE P.ID_MESSEGE='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    public function GetTextByMessegeID($ID, $EVENT_ID, $COUNT_RUN)
    {
        ...
        if(empty($arRes['MESSEGE'])) {
            ...
            $eventTemplate = CKitMailingHelp::GetEventTemplate($EVENT_ID);
            $TEMPLATE = '';            
            ...
            if($TEMPLATE){
                ...
                $arRes['MESSEGE'] = CKitMailingHelp::ReplaceVariables($arRes['MESSEGE'], $arRes['MESSEGE_PARAMETR']);
            }            
    ...
    public function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_kit_mailing_message_text WHERE ID='".$DB->ForSql($ID)."'";
    ...
    public function GetList($aSort=Array(), $arFilter=Array(), $arNavStartParams=false, $arSelect=Array())
    {
        ...
        $strSql = "
            SELECT 
                 ".$strSqlSelect."            
            FROM b_kit_mailing_message_text P
            WHERE
            ".$strSqlSearch."
        ";   
