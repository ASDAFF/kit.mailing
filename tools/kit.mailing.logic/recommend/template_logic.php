...
if($arParams["RECOMMEND_SHOW"] == 'Y' && CModule::IncludeModule("iblock")) {
    ...
    if($arParams['INCLUDE_PHP_MODIF_RECOMMEND']=='Y'){
        include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/kit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_RECOMMEND_FILLTER_BEFORE.php");
    }    
    ...
    {      
        ...
        if($arParams['INCLUDE_PHP_MODIF_RECOMMEND']=='Y'){ 
            include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/kit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_RECOMMEND_FILLTER_WHILE_AFTER.php");
        }
    ...
    if(count($arrRecommendProduct) > 0) {
        ...
        foreach($arrRecommendProduct as $ItemRecommendProduct) {
            ...
            $RECOMMEND_PRODUCT .= CKitMailingHelp::ReplaceVariables($arParams["TEMP_LIST_RECOMMEND"] , $ItemRecommendProduct);
            $i++;           
        }
