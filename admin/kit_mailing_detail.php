...
//START
$arrTempl = array();
$templComponent = CComponentUtil::GetTemplatesList('kit:kit.mailing.logic', true);
...
{
    LocalRedirect("kit_mailing_list.php?lang=".LANG, true);
}
...
if($_GET['ID'])
{
    ...
    $ListMailing = CKitMailingHelp::GetMailingInfo();
...
if($_REQUEST['update'] && $_REQUEST['save'])
{
    ...
    if($arResult['ID'])
    {
        CKitMailingEvent::Update($arResult['ID'] , $arr_save);

        // �������� ������ � ��� ���������
        // START
        $arrEventInfo = CKitMailingEvent::GetByID($arResult['ID']);
        $arrEventInfo['TEMPLATE_PARAMS'] = unserialize($arrEventInfo['TEMPLATE_PARAMS']);
        $resEventTemplate = CKitMailingMessageTemplate::GetList(array('COUNT_START' => 'DESC'), array('ID_EVENT' => $arResult['ID'], 'ARCHIVE' => 'N'), false, array());
        while($arrEventTemplate = $resEventTemplate->Fetch())
        {
            ...
            {
                CKitMailingMessageTemplate::Update($arrEventTemplate['ID'], array('TEMPLATE' => $arrEventInfo['TEMPLATE_PARAMS']['MESSAGE']));
                break;         
            } 
            ...
            {
                CKitMailingMessageTemplate::Update($arrEventTemplate['ID'],array('ARCHIVE'=>'Y','COUNT_END'=>$arrEventInfo['COUNT_RUN']));
    ...
    else
    {
        ...
        //��� ��������� ������ ��������� �������, ������ �������� ������ �� ���������
        //$arr_save['EVENT_TYPE'] = CKitMailingHelp::EventMessageCheck();
        $arr_save['EVENT_TYPE'] = '';

        $arResult['ID'] = CKitMailingEvent::Add($arr_save);
        ...
        //�������� ��� ��������� ������� � ��������
        CKitMailingEvent::Update($arResult['ID'], array('EVENT_TYPE' => $eventType));
    ...
    {
        $arResult['AGENT_ID'] = CAgent::AddAgent('CKitMailingTools::AgentStartTemplate('.$arResult['ID'].');', "kit.mailing", "N", '3600', "", "N");
        $_REQUEST['AGENT_ID'] = $arResult['AGENT_ID'];
        CKitMailingEvent::Update($arResult['ID'], array('AGENT_ID' => $arResult['AGENT_ID']));
    }
    ...
    if($_REQUEST['AGENT_ID'] && $arResult['ID'])
    {
        ...
        $arrAgentUpdate['NAME'] = 'CKitMailingTools::AgentStartTemplate('.$arResult['ID'].');';
        $arrAgentUpdate['MODULE_ID'] = 'kit.mailing';
    ...
    LocalRedirect("kit_mailing_detail.php?parent=".$parentID."&ID=".$arResult['ID']."&KIT_MAILING_DETAIL=Y&lang=".LANG."&tabControl_active_tab=".$_REQUEST['tabControl_active_tab'].'#form', true);
}
// END
...
if($arResult['ID'] > 0)
{
    if($res = CKitMailingEvent::GetByID($arResult['ID']))
    {
        ...
        //�������� ��������� ������� ������ �� ���������
        /*$EVENT_TYPE = CKitMailingHelp::EventMessageCheck();
        if($res['EVENT_TYPE'] != $EVENT_TYPE) {
            CKitMailingEvent::Update($res['ID'] , array('EVENT_TYPE' => $EVENT_TYPE));
        } */
        ...
        if($res['EVENT_SEND_SYSTEM'] == 'UNISENDER')
        {
            ...
            if(empty($arrListUniSender[$res['EVENT_SEND_SYSTEM_CODE']])){
                ...
                $createListUniSender = CKitMailingHelp::QueryUniSender('createList', array('title'=>$res['NAME']));
                if($createListUniSender['result']['id']){
                     CKitMailingEvent::Update($res['ID'] , array('EVENT_SEND_SYSTEM_CODE' => $createListUniSender['result']['id']));
                }             
            }
            //���� ���������� ��������, �������
            if($arrListUniSender[$res['EVENT_SEND_SYSTEM_CODE']] && $arrListUniSender[$res['EVENT_SEND_SYSTEM_CODE']] != $res['NAME']) {
                $updateListUniSender = CKitMailingHelp::QueryUniSender('updateList', array('list_id'=>$res['EVENT_SEND_SYSTEM_CODE'],'title'=>$res['NAME']));
            }    
            //END
        }
        //END
        ...
        if($res['AGENT_ID'])
        {
            ...
            {
                $res['AGENT_ID'] = CAgent::AddAgent('CKitMailingTools::AgentStartTemplate('.$arResult['ID'].');', "kit.mailing", "N", '3600', "", "N");
                CKitMailingEvent::Update($res['ID'], array('AGENT_ID' => $res['AGENT_ID']));
            }
        }
        // END
...
elseif($_GET['ID_COPY']) // ���� �������� ���������� �����������
{
    if($res = CKitMailingEvent::GetByID($_GET['ID_COPY']))
    {
        $res['TEMPLATE_PARAMS'] = unserialize($res['TEMPLATE_PARAMS']);
        $res['EXCLUDE_UNSUBSCRIBED_USER_MORE'] = unserialize($res['EXCLUDE_UNSUBSCRIBED_USER_MORE']);
...
$aMenu = array(
    array(
        ...
        "ICON" => "btn_list",
        //"LINK" => "/bitrix/admin/kit_mailing_list.php?lang=".LANG.GetFilterParams("filter_")
        "LINK" => "/bitrix/admin/kit_mailing_list.php?parent=".$parentID."&lang=".LANG.GetFilterParams("filter_")
    )
);
...
if($ID)
{
    ...
    if(is_array($arrTempl))
    {
        foreach($arrTempl as $kres => $arRes)
        {
             $arDDMenu[] = array(
                ...
                "ACTION" => "window.location = 'kit_mailing_detail.php?parent=".$parentID."&lang=".LANG."&TEMPLATE=".$arRes["NAME"]."';"
            );
        }
    }
    ...
    {
        $arrProgress = CKitMailingHelp::ProgressFileGetArray($arResult['ID'], $arResult['COUNT_RUN']);
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('".GetMessage($module_id."_PANEL_TOP_DELETE_CONFORM",array("#ID#"=>$ID))."')) window.location='/bitrix/admin/kit_mailing_list.php?parent=".$parentID."&action=delete&ID[]=R".$ID."&lang=".LANG."&".bitrix_sessid_get()."#tb';",
...
$_arCategory = CKitMailingSectionTable::getCategoryList();
...
$paramTemplate = CComponentUtil::GetTemplateProps('kit:kit.mailing.logic', $arResult['TEMPLATE'], "", $arResult['TEMPLATE_PARAMS']);
...
$("#btn_start_send").on('click', function(e) {
    ...
    BX.ajax.get("/bitrix/admin/kit_mailing_start_ajax.php?ID=<?=$ID?>&KIT_MAILING_DETAIL=Y&MAILING_START=Y&COUNT_RUN=<?=$arResult['COUNT_RUN']?>", "", function(data){});
...
$("#btn_stop_send").on('click', function(e) {
    ...
    BX.ajax.get("/bitrix/admin/kit_mailing_start_ajax.php?ID=<?=$ID?>&KIT_MAILING_DETAIL=Y&MAILING_STOP=Y&COUNT_RUN=<?=$arResult['COUNT_RUN']?>", "", function(data){});
...
function KitGetJsonProgress()
{
    $('#status_bar').show();
    
    $.ajaxSetup({cache: false});
    
    $.getJSON('/bitrix/tmp/kit_mailing_progress_<?=$ID?>_<?=$arResult['COUNT_RUN']?>.json', function(data) {
        if(data.COUNT_NOW>0) {
        ...
        if(data.EMAIL_TO_SEND) {                                                                                          
            BX('progress_info_mail_send').innerHTML = "<?=GetMessage($module_id.'_JS_SEND_MAILING')?> "+data.EMAIL_TO_SEND+" <a target='_blank' href='/bitrix/tmp/kit_mailing_progress_<?=$ID?>_<?=$arResult['COUNT_RUN']?>_EMAIL_TO_SEND.txt'><?=GetMessage($module_id.'_JS_MORE_INFO')?></a><br />";
        }
        if(data.EMAIL_TO_EXCLUDE_UNSUBSCRIBED) {
            BX('progress_info_mail_exclude_unsubscribed').innerHTML = "<?=GetMessage($module_id.'_JS_NO_SEND_EXCLUDE_UNSUBSCRIBED_MAILING')?> "+data.EMAIL_TO_EXCLUDE_UNSUBSCRIBED+" <a target='_blank' href='/bitrix/tmp/kit_mailing_progress_<?=$ID?>_<?=$arResult['COUNT_RUN']?>_EMAIL_TO_EXCLUDE_UNSUBSCRIBED.txt'><?=GetMessage($module_id.'_JS_MORE_INFO')?></a> <br />";
        } 
        if(data.EMAIL_TO_EXCLUDE_UNDELIVERED) {
            BX('progress_info_mail_exclude_undelivered').innerHTML = "<?=GetMessage($module_id.'_JS_NO_SEND_EXCLUDE_UNDELIVERED_MAILING')?> "+data.EMAIL_TO_EXCLUDE_UNDELIVERED+" <a target='_blank' href='/bitrix/tmp/kit_mailing_progress_EMAIL_TO_EXCLUDE_UNDELIVERED.txt'><?=GetMessage($module_id.'_JS_MORE_INFO')?></a> <br />";
        }               
        if(data.EMAIL_TO_EXCLUDE_HOUR_AGO) {
            BX('progress_info_mail_exclude_hour_ago').innerHTML = "<?=GetMessage($module_id.'_JS_NO_SEND_EXCLUDE_HOUR_AGO_MAILING')?> "+data.EMAIL_TO_EXCLUDE_HOUR_AGO+" <a target='_blank' href='/bitrix/tmp/kit_mailing_progress_<?=$ID?>_<?=$arResult['COUNT_RUN']?>_EMAIL_TO_EXCLUDE_HOUR_AGO.txt'><?=GetMessage($module_id.'_JS_MORE_INFO')?></a> <br />";
        }  
        ...
        if(data.MAILING_WORK == 'N') {
            BX.ajax.get("/bitrix/admin/kit_mailing_start_ajax.php?ID=<?=$ID?>&KIT_MAILING_DETAIL=Y&MAILING_START=Y&COUNT_RUN=<?=$arResult['COUNT_RUN']?>&MAILING_WORK_CHECK=Y", "", function(data){});
        }
    });
...
<script>
    KitGetJsonProgress();
    <?
    $arrProgress = CKitMailingHelp::ProgressFileGetArray($ID, $arResult['COUNT_RUN']);
