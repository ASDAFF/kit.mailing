...
/**
* Вспомогательный класс модуля kit.mailing
*
* Константы класса:
* <ul>
...
class CKitMailingHelp
{
    ...
    /**
    * Метод включает управляемый кэш инфоблоков, если в настройках модуля kit.mailing установлен соответсвующий параметр.
    * 
    * @return void
    *
    */
    ...
    {
        if(COption::GetOptionString("kit.mailing", "MANAGED_CACHE_ON", "Y") == "Y")
        {
            define("BX_COMP_MANAGED_CACHE", true);
        } 
        elseif(COption::GetOptionString("kit.mailing", "MANAGED_CACHE_ON") == "N")
        {
    ...
    */
    public function GetMailingInfo()
    {
        ...
        {     
           ...
           $resData = CKitMailingEvent::GetList(array('ID'=>'ASC'),array(),false,$select);
    ...
    public function GetCategoriesInfo()
    {
        ...
        {     
           ...
           $resData = CKitMailingCategories::GetList(array('ID'=>'ASC'),array(),false, array());
           while($arrData = $resData->Fetch()) {
                $vars[$arrData['ID']] = $arrData;                
           }           
    ...
    public function GetEventTemplate($ID_EVENT)
    {
        ...
        {     
           ...
           $resData = CKitMailingMessageTemplate::GetList(array('COUNT_START'=>'DESC'),array('ID_EVENT' => $ID_EVENT) ,false,array()); 
           while($arrData = $resData->Fetch()) {
                $vars[$arrData['ID']] = $arrData;                
           }          
    ...
    */
    public function GetEmailSendMessageTimeNoCache($SETTING=array())
    {
        ...
        $GetMailingInfo = CKitMailingHelp::GetMailingInfo();    
        ...
        $resData = CKitMailingMessage::GetList(array(),$FillterMessage,false,array('ID','EMAIL_TO','DATE_SEND'));
        while($arrData = $resData->Fetch()) {
    ...
    public function GetEmailSendMessageTime($SETTING=array())
    {
        ...
        {     
           ...
           $vars = array();
           $vars = CKitMailingHelp::GetEmailSendMessageTimeNoCache($SETTING);
    ...
    */
    ...
    {
        $vars = array();
        $resSearch = CKitMailingUnsubscribed::GetList(array(), array('ID_EVENT' => $ID_EVENT, "ACTIVE" => "Y"), false, array('ID','EMAIL_TO'));
    ...
    public function GetUnsubscribedByMailing($ID_EVENT=false)
    {
        ...
        {     
           ...
           $vars = array();
           $vars = CKitMailingHelp::GetUnsubscribedByMailing_NoCache($ID_EVENT);            
    ...
    */
    ...
    {
        $vars = array();
        $resSearch = CKitMailingUnsubscribed::GetList(array(), array("ACTIVE" => "Y"), false, array('ID','EMAIL_TO'));
        while ($arrSearch = $resSearch->Fetch())
        {
            $arrSearch['EMAIL_TO'] = trim($arrSearch['EMAIL_TO']);               
            $vars[$arrSearch['EMAIL_TO']] = $arrSearch['ID'];  
    ...
    public function GetUnsubscribedAllMailing()
    {
        ...
        {     
           ...
           $vars = array();
           $vars = CKitMailingHelp::GetUnsubscribedAllMailing_NoCache();           
    ...
    */
    ...
    {
        $vars = array();
        $resSearch = CKitMailingUndelivered::GetList(array(), array("ACTIVE" => "Y"), false, array('ID','EMAIL_TO'));
        while ($arrSearch = $resSearch->Fetch())
        {
            $arrSearch['EMAIL_TO'] = trim($arrSearch['EMAIL_TO']);               
            $vars[$arrSearch['EMAIL_TO']] = $arrSearch['ID'];  
    ...
    public function GetUndeliveredMailing()
    {
        ...
        {     
           ...
           $vars = array();
           $vars = CKitMailingHelp::GetUndeliveredMailing_NoCache();           
    ...
    */
    ...
    {
        $eventTypeName = 'KIT_MAILING_EVENT_' . $mailingId;
    ...
    public function ReplaceTemplateLinks($messageText, $siteUrl = '')
    {
        ...
        foreach($arr_link as $k=>$v){    
            ...
            if($parse_url['host']!=$parse_url_mailing['host']){
                $arr_link_need[$k] = 'href='.$find[1][$k].$siteUrl.'/?MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=kit_mailing_#MAILING_EVENT_ID#&MAILING_FILE_REDURECT='.$v.$find[1][$k];
            } 
            else {
                if(strpos($v,'?')){
                    $arr_link_need[$k] = 'href='.$find[1][$k].$v.'&MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=kit_mailing_#MAILING_EVENT_ID#'.$find[1][$k];
                } else {
                    $arr_link_need[$k] = 'href='.$find[1][$k].$v.'?MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=kit_mailing_#MAILING_EVENT_ID#'.$find[1][$k];
                }             
            } 
    ...
    */    
    public function MessageCheck($SETTING)
    {
        ...
        // START 
            ...
            if($arParams['MAILING_EXCLUDE_HOUR_AGO'] != 0) { 
                ...
                $HourAgoEmail = CKitMailingHelp::GetEmailSendMessageTimeNoCache($setting_HourAgoEmail);   
        ...
        if($arParams['MAILING_EXCLUDE_UNSUBSCRIBED_USER'] == 'ALL') {
            $UnsubscribeEmail = CKitMailingHelp::GetUnsubscribedAllMailing_NoCache();  
        } 
        elseif($arParams['MAILING_EXCLUDE_UNSUBSCRIBED_USER'] == 'THIS') {
            $UnsubscribeEmail = CKitMailingHelp::GetUnsubscribedByMailing_NoCache($arParams["MAILING_EVENT_ID"]);  
            //добавим адреса на отписку 
            foreach($arParams['MAILING_EXCLUDE_UNSUBSCRIBED_USER_MORE'] as $event_id) {
                $UnsubscribeEmailMore = CKitMailingHelp::GetUnsubscribedByMailing_NoCache($event_id);        
        ...
        //START   
        $UndeliveredEmail = CKitMailingHelp::GetUndeliveredMailing_NoCache();  
        ...
        CKitMailingHelp::ProgressFile($arParams["MAILING_EVENT_ID"], $arParams['MAILING_COUNT_RUN'], $arrProgress, $arrProgress_more);     
    ...
    public function QueryUniSender($Metod=false, $Fields=array())
    {
       ...
       // выставим ключ
       $data = array(
          'api_key' => COption::GetOptionString('kit.mailing', 'UNSENDER_API_KEY'),
          'format' => 'json',
          'double_optin' => '1'
       );
    ...
    public function UniSenderExportContact($CATEGORIES_ID, $LIST_UNISENDER)
    {
        ...
        while($d <= 1000000)
        {
            ...
            $ExportList = CKitMailingHelp::QueryUniSender('exportContacts',$arrUniseder);    
        ...
        foreach($arrayUser as $exportInfo) {
            ...
            $ANSWER = CkitMailingSubTools::AddSubscribers($exportInfo, array('UNISENDER_EXPORT'=>'Y'));
    ...
    */
    ...
    {
        $file_path =  $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN.".json";
        // создадим папку
        ...
        if($MORE_INFO['EMAIL_TO_SEND_INFO']){
            ...
            $file = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_SEND.txt";
        ...
        if($MORE_INFO['EMAIL_TO_EXCLUDE_HOUR_AGO_INFO']){
            ...
            $file = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_EXCLUDE_HOUR_AGO.txt";
            $current = file_get_contents($file);
        ...
        if($MORE_INFO['EMAIL_TO_EXCLUDE_UNSUBSCRIBED_INFO']){
            ...
            $file = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_EXCLUDE_UNSUBSCRIBED.txt";
            $current = file_get_contents($file);
        ...
        if($MORE_INFO['EMAIL_TO_EXCLUDE_UNDELIVERED_INFO']){
            ...
            $file = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_EMAIL_TO_EXCLUDE_UNDELIVERED.txt";
            $current = file_get_contents($file);
    ...
    */
    public function ProgressFileDelete($EVENT_ID=0, $EVENT_COUNT_RUN=0)
    {
        ...
        $file_path = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_SEND.txt";
        if(file_exists($file_path))
            unlink($file_path);
        
        $file_path = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_EXCLUDE_HOUR_AGO.txt";
        if(file_exists($file_path))
            unlink($file_path);
        
        $file_path = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_".$EVENT_ID."_".$EVENT_COUNT_RUN."_EMAIL_TO_EXCLUDE_UNSUBSCRIBED.txt";
        if(file_exists($file_path))
            unlink($file_path);
        
        $file_path = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tmp/kit_mailing_progress_EMAIL_TO_EXCLUDE_UNDELIVERED.txt";
        if(file_exists($file_path))
            unlink($file_path);
    }
    ...
    */
    public function GetNeedVariablesTemplate($TEMPLATE)
    {
        ...
        foreach($chars as $kte => $vte) {
            $countVte = strlen($vte);
            if($countVte < 150 && CKitMailingHelp::multineedle_stripos($vte,$simvolNone)==false){
               $templateArr[$vte] = $vte;
            }
        }
    ...
    * 
    * Метод подклюдчает файлы event.php из папок шаблонов компонента kit.mailing.logic, если они существуют
    ...
    */   
    public function OnPageStart()
    {
        ...
        $templComponentMailing = CComponentUtil::GetTemplatesList('kit:kit.mailing.logic',true);
    
        if(is_array($templComponentMailing)) { 
            foreach($templComponentMailing as $valTemp){    
                if($valTemp['NAME'] != '.default') {   
                    if($valTemp['TEMPLATE']) {
                        $fileEventName = '/bitrix/templates/'.$valTemp['TEMPLATE'].'/components/kit/kit.mailing.logic/'.$valTemp['NAME'].'/event.php';
                        if(file_exists($_SERVER["DOCUMENT_ROOT"].$fileEventName)) {
                            $arrEventLogicFile[$valTemp['NAME']] = $fileEventName;    
                        }                                 
                    } 
                    else {
                        $fileEventName = '/bitrix/components/kit/kit.mailing.logic/templates/'.$valTemp['NAME'].'/event.php';
                        if(file_exists($_SERVER["DOCUMENT_ROOT"].$fileEventName)) {
                            $arrEventLogicFile[$valTemp['NAME']] = $fileEventName;    
                        } 
                    }                
        ...
        $ar_Temp_id = array();
        $ar_activeTemp = array();
        $db_activeTemp = CKitMailingEvent::GetList(array(),array('ACTIVE'=>'Y'),false,array('ID','TEMPLATE','TEMPLATE_PARAMS'));
        ...
        if($_GET['USER_AUTH'] && $_GET['MAILING_MESSAGE']) { 
            ...
            if(!$USER->IsAuthorized()) {
                ...
                $paramMessage = CKitMailingMessage::GetByIDInfoParamMessage($_GET['MAILING_MESSAGE']); 
                if($paramMessage['USER_AUTH']==$_GET['USER_AUTH'] && !empty($paramMessage['USER_ID'])){

					$auth = false;
					$groups = unserialize(Main\Config\Option::get('kit.mailing','AUTH_GROUPS','a:1:{i:0;s:1:"6";}'));
                	if(!is_array($groups))
					{
						$groups = [];
					}
...
//                }
                ...
                if(!empty($user_came_where_redirect)) {
                    ...
                    $user_came_where_redirect .= ($query ? '&' : '?').'KIT_MAILING_STOP_ADD_BASKET=Y';
                }
        ...
        if($_GET['MAILING_UNSUBSCRIBE']) { 
            ...
            if($ID_MESSEGE && $ID_EVENT) {
                ...
                $MessageInfo = CKitMailingMessage::GetByID($ID_MESSEGE);
                if($MessageInfo['ID_EVENT'] == $ID_EVENT && !empty($MessageInfo['EMAIL_TO'])) {
                    global $DB;
                    $EMAIL_TO = $MessageInfo['EMAIL_TO'];
         
                    $resSearch = CKitMailingUnsubscribed::GetList(array(), array('ID_EVENT' => $ID_EVENT, 'EMAIL_TO' => $EMAIL_TO));
                    $arrSearch = $resSearch->Fetch();
                    if(empty($arrSearch)) {
                     
                        CKitMailingUnsubscribed::Add(array(
    ...
    * 
    * Метод записывает номер заказа в сообщение (таблица b_kit_mailing_message)
    *
    * @return void
    *
    */
    public function OnOrderAdd($ID, $arFields)
    {
        ...
        {
            CKitMailingMessage::Update($_COOKIE["MAILING_CAME_MESSAGE_ID"], array('STATIC_ORDER_ID' => $ID));
        }
    }
